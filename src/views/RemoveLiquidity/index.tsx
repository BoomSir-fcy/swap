...
import { Currency, currencyEquals, ETHER, Percent, WETHER } from "swap-sdk";
import {
  Button,
  Text,
  AddIcon,
...
export default function RemoveLiquidity() {
  ...
  const oneCurrencyIsWETH = Boolean(
    chainId &&
      ((currencyA && currencyEquals(WETHER[chainId], currencyA)) ||
        (currencyB && currencyEquals(WETHER[chainId], currencyB)))
  );
    ...
    <Page>
      <AppBody>
        <AppHeader
          ...
          subtitle={t("To receive %symbol% and %symbol2%", {
            symbol: currencyA?.symbol ?? "",
            symbol2: currencyB?.symbol ?? "",
          })}
          noConfig
        />
                    ...
                    >
                      {oneCurrencyIsETH ? (
                        ...
                        <StyledInternalLink
                          to={`/remove/${
                            currencyA &&
                            currencyEquals(currencyA, WETHER[chainId])
                              ? ETHER.symbol
                              : currencyIdA
                          }/${
                            currencyB &&
                            currencyEquals(currencyB, WETHER[chainId])
                              ? ETHER.symbol
                              : currencyIdB
                          }`}
                        >
