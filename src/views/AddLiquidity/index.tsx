...
export default function AddLiquidity() {
  ...
  const routerAddress = React.useMemo(() => {
    const TokenAmountA = parsedAmounts[Field.CURRENCY_A];
    const TokenAmountB = parsedAmounts[Field.CURRENCY_B];
    const tokenA =
      TokenAmountA instanceof TokenAmount ? TokenAmountA.token : undefined;
    const tokenB =
      TokenAmountB instanceof TokenAmount ? TokenAmountB.token : undefined;

    if (
      tokenA &&
      ROUTER_WHITE_TOKEN.some(
        (item) => tokenA.address.toLowerCase() === item.toLowerCase()
      )
    ) {
      return ROUTER_ADDRESS_WHITE;
    }
    if (
      tokenB &&
      ROUTER_WHITE_TOKEN.some(
        (item) => tokenB.address.toLowerCase() === item.toLowerCase()
      )
    ) {
      return ROUTER_ADDRESS_WHITE;
    }
  ...
  async function onAdd() {
    ...
    const router = getRouterContract(chainId, library, account, routerAddress);
    ...
    <Page>
      <AppBody>
        <AppHeader
          ...
          noConfig
          helper={t(
            "Liquidity providers earn a 0.1% trading fee on all trades made for that token pair, proportional to their share of the liquidity pool."
          )}
          backTo="/pool"
        />
        <CardBody>
          <AutoColumn gap="20px">
            ...
            />
            <ColumnCenter>
              <AddIcon color="textPrimary" width="32px" />
            </ColumnCenter>
            <CurrencyInputPanel
            ...
            {addIsUnsupported ? (
              <Flex justifyContent="center">
                <Button scale="ld" disabled mb="4px">
                  {t("Unsupported Asset")}
                </Button>
              </Flex>
            ) : !account ? (
